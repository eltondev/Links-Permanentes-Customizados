<?php
/*
Plugin Name: Links Permanentes Customizados
Plugin URI: https://github.com/eltondev/Links-Permanentes-Customizados
Description: Você pode criar URLs customizadas para: posts, produtos, páginas e etc...
Version: 0.0.1
Author: EltonDEV
Author URI: http://eltondev.com.br
Donate link: http://eltondev.com.br/doacao/
Text Domain: links-custom-build
*/
...
/**
 * Action to redirect to the Links Permanentes
 *
 * @package CustomPermalinks
 * @since 0.14
 */
function custom_permalinks_redirect() {
  ...
  // If the post/tag/category we're on has a Links Permanentes, get it and check against the request
...
function custom_permalinks_request($query) {
  ...
  // First, search for a matching Links Permanentes, and if found, generate the corresponding
  // original URL
...
function custom_permalink_get_sample_permalink_html($html, $id, $new_title, $new_slug) {
  ...
  ob_end_clean();
    ...
    if ( 'publish' == $post->post_status ) {
        $view_post = 'page' == $post->post_type ? __('View Page', 'links-custom-build') : __('View Post', 'links-custom-build');
  }
  ...
  return '<strong>' . __('Permalink:', 'links-custom-build') . "</strong>\n" . $content .
...
function custom_permalinks_post_options() {
  ...
  <h3><?php _e('Links Permanentes', 'links-custom-build') ?></h3>
  <div class="inside">
...
function custom_permalinks_form($permalink, $original="", $renderContainers=true) {
  ?>
  <input value="true" type="hidden" name="permalinks_edit_custom" />
  ...
  <tr>
    <th scope="row"><?php _e('Links Permanentes', 'links-custom-build') ?></th>
    <td>
  <?php endif; ?>
      <?php echo home_url() ?>/
  ...
  <?php if ( $renderContainers ) : ?>
      <br />
      <small><?php _e('Deixe em branco para desabilitar', 'links-custom-build') ?></small>
...
function custom_permalinks_save_post($id) {
  if ( !isset($_REQUEST['permalinks_edit_custom']) ) return;
...
function custom_permalinks_save_tag($id) {
  if ( !isset($_REQUEST['permalinks_edit_custom']) || isset($_REQUEST['post_ID']) ) return;
...
function custom_permalinks_options_page() {
  ...
  if ( isset($_REQUEST['revertit']) && isset($_REQUEST['revert']) ) {
    check_admin_referer('links-custom-build-bulk');
  ...
  ?>
  <div class="wrap">
  <h2><?php _e('Links Permanentes', 'links-custom-build') ?></h2>
  
  <form method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>">
  <?php wp_nonce_field('links-custom-build-bulk') ?>
  
  <div class="tablenav">
  <div class="alignleft">
  <input type="submit" value="<?php _e('Reverter', 'links-custom-build'); ?>" name="revertit" class="button-secondary delete" />
  </div>
  <br class="clear" />
  </div>
  <br class="clear" />
  <table class="widefat">
    ...
    <tr>
      ...
      <th scope="col"><?php _e('Título', 'links-custom-build') ?></th>
      <th scope="col"><?php _e('Tipo', 'links-custom-build') ?></th>
      <th scope="col"><?php _e('Permanete Link', 'links-custom-build') ?></th>
    </tr>
    </thead>
    <tbody>
  <?php
  ...
  foreach ( $rows as $row ) {
    ...
    <td><a href="<?php echo $row['permalink'] ?>" target="_blank" title="<?php printf(__('Visit %s', 'links-custom-build'), htmlspecialchars($row['title'])) ?>">
...
function custom_permalinks_setup_admin_menu() {
  add_management_page( 'Links Permanentess', 'Links Permanentess', 'edit_others_pages', 'custom_permalinks', 'custom_permalinks_options_page' );
}
...
# "Fatal error: Call to undefined function get_bloginfo() in C:\xampp\htdocs\links-custom-build\links-custom-build.php on line 753"
